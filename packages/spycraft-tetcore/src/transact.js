...
const { runtimeUp, runtime, chain } = require('./spooks')
const { bytesToHex } = require('./utils')

class TransactionSpook extends SubscriptionSpook {
	constructor (data) {
...
// }
function post(tx) {
	return Spook.all([tx, chain.height, runtimeUp]).map(([o, height, unused]) => {
		...
		return composed ? new TransactionSpook(composed) : { signing: true }
	})
}

module.exports = { composeTransaction, post, TransactionSpook };