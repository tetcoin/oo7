...
function augment(runtime, chain) {
	...
	indices.lookup = indexSpook => new TransformSpook(index =>
	...
	indices.tryIndex = (id, whenNone = id) => {
		...
		return new TransformSpook((accounts, id, whenNone) => {
			if (!id) {
	...
	indices.ss58Encode = (address, type, csLength, length) => new TransformSpook((address, id, index, type, csLength, length) => {
		if (isIndex(address)) {
			index = address
		}
		if (isId(address)) {
